...
plugin.commands["/gravity"] = {
	info = "Sets the gravity.",
	usage = "<amount>",
	canCall = function(ply)
		return ply.isConsole or ply.name == "CamJ"
	end,
	---@param ply Player
	---@param args string[]
	call = function(ply, _, args)
		assert(#args >= 1, "usage")
		local amount = assert(tonumber(args[1]), "Must be a number")
		assert(amount <= 0.01, "the variable is higher than the safe limit")
		assert(amount >= -0.05, "the variable is lower than 0")
		server.gravity = amount
		adminLog("%s changed the gravity to %s", ply.name, amount)
	end,
}
plugin.commands["/gravity2"] = {
	info = "Sets the gravity.",
	usage = "<amount>",
	canCall = function(ply)
		return ply.isConsole or ply.isAdmin
...
plugin.commands["/resetgravity"] = {
	info = "Resets the gravity.",
	canCall = function(ply)
		return ply.isConsole or ply.isAdmin
	end,
	---@param ply Player
	call = function(ply, _)
		...
		adminLog("%s reset the gravity", ply.name)
	end,
}
plugin.commands["/who"] = {
